...
type HttpClient interface {
	...
	Post(url string, config *PostConfig, responseData any) error
}
...
func (c *httpClient) Post(url string, config *PostConfig, responseData any) error {
	request, err := http.NewRequest(http.MethodPost, url, bytes.NewBuffer(config.Body))
	if err != nil {
		return err
	}
